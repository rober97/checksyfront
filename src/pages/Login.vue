  ...
  >
    ...
    <div class="rk-center">
      <q-card class="login-card" flat bordered>
        ...
        <!-- Form -->
        <q-form @submit.prevent="handleLogin" class="q-gutter-md q-px-lg q-pb-md">
          <q-input
            ref="emailRef"
            v-model="email"
          ...
          >
            ...
            <template #hint>
              <span v-if="capsOn" class="text-negative text-bold">⚠ Caps Lock activado</span>
            </template>
          </q-input>
        ...
        <!-- Footer -->
        <q-card-section class="text-center text-caption q-pb-lg"
...
function forgotPassword () { console.info('Recuperación de contraseña') }
function soporte () { window.open('mailto:soporte@recksy.cl', '_blank') }
function onPasswordKeyUp (e) { capsOn.value = e.getModifierState && e.getModifierState('CapsLock') }
...
async function handleLogin () {
  ...
  if (!emailOk || !pwdOk) { formError.value = 'Revisa los campos del formulario.'; return }
  if (!navigator.onLine) { formError.value = 'Sin conexión a internet.'; return }
...
let ctx, W, H, rafId, lastT = 0, spawnAcc = 0
...
function makeParticle (edge = false) {
  ...
  if (edge) {
    ...
    if (side === 0) { x = -8; y = rand(0, H) }         // izquierda
    if (side === 1) { x = W + 8; y = rand(0, H) }      // derecha
    if (side === 2) { x = rand(0, W); y = -8 }         // arriba
    if (side === 3) { x = rand(0, W); y = H + 8 }      // abajo
  }
...
function respawn (p, edge = true) { Object.assign(p, makeParticle(edge)) }
...
function step (now) {
  ...
  for (let i = 0; i < particles.length; i++) {
    for (let j = i + 1; j < particles.length; j++) {
      ...
      if (d2 < 120 * 120) {
        ...
        const alpha = (1 - d2 / (120 * 120)) * (inMouse ? 0.10 : 0.32)
  ...
  // Puntos
  for (const p of particles) {
    ctx.fillStyle = isDark.value ? 'rgba(160,200,255,0.9)' : 'rgba(70,120,200,0.9)'
...
function onMouseMove (e) {
  ...
  mouseX.value = MOUSE.x; mouseY.value = MOUSE.y
}
function onMouseLeave () {
  MOUSE.x = -9999; MOUSE.y = -9999
  mouseX.value = -9999; mouseY.value = -9999
}
